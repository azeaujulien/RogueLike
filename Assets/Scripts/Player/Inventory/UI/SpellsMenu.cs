...
public class SpellsMenu : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        float iconDist = Vector3.Distance(ringCakePiecePrefab.icon.transform.position,
            ringCakePiecePrefab.cakePiece.transform.position);
        ...
        for (int i = 0; i < spells.Length; i++) {
            ...
            _pieces[i].cakePiece.transform.localRotation =
                Quaternion.Euler(0, 0, -stepLength / 2f + gapWidthDegree / 2f + i * stepLength);
            _pieces[i].cakePiece.color = new Color(1f, 1f, 1f, 0.5f);

            //set icon
            _pieces[i].icon.transform.localPosition = _pieces[i].cakePiece.transform.localPosition +
    ...
    /// <summary>
    ///     This function normalize the angle given in parameter
    /// </summary>
